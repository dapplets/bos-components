...
const TIPPING_CONTRACT_NAME =
  context.networkId === 'mainnet' ? 'v2.tipping.near' : 'v2.tipping.testnet'
...
const createLabel = () => {
  if (state.totalTipsByItemId === '0' && state.amount === '0')
    return state.isHovered ? '+ ' + STEP : 'Tip'
  if (state.amount === '0') {
    return state.isHovered ? '+ ' + STEP : formatNear(state.totalTipsByItemId)
  } else {
    return (
      '+ ' +
      (state.isHovered ? formatNear(sum(state.amount, STEP_YOCTO)) : formatNear(state.amount))
    )
  }
}
...
return (
  ...
  >
    ...
    <div className="icon">
      <Widget loading={<></>} src="${REPL_ACCOUNT}/widget/Tipping.TipIcon" />
    </div>
    <div className="cost">
      {createLabel()}
      <Widget loading={<></>} src="${REPL_ACCOUNT}/widget/Tipping.NearIcon" />
    </div>
  </TippingButton>
)
