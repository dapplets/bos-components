...
const LabelImage = styled.div`
  ...
  left: 10px;
  box-shadow:
    0px 3px 6px 0px rgba(24, 124, 205, 0.25),
  ...
  <>
    {state.img.cid && state.isUpload ? (
      <WrapperImage>
        ...
        <img src={`https://ipfs.near.social/ipfs/${state.img.cid}`} alt="uploaded" />
      </WrapperImage>
    ) : (
      <WrapperWidget>
        ...
        <InputsBlock>
          ...
          <BlockAmount>
            ...
            <InputAmount value={state.amount ? state.amount : ''} onChange={amountOnChange} />
          </BlockAmount>
        </InputsBlock>
        <ButtonsBlock>
          <ButtonCancel onClick={cancelOnChange}>Cancel</ButtonCancel>
          <ButtonSend onClick={sendOnChange} disabled={!state.img || !state.amount}>
            Send
          </ButtonSend>
        </ButtonsBlock>
      </WrapperWidget>
